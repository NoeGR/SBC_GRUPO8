...
esp_http_client_handle_t cliente_http;  
...
#define WIFIB_CONNECTED_IT BIT0
...
static void https_telegram_getUpdates(char *respuesta) {
    ...
    ESP_LOGI(TAG_HTTP, "Recibire GET");
    ...
    if (mensaje_error == ESP_OK) {
        ESP_LOGI(TAG_HTTP, "HTTPS Status = %d, content_length = %lld",
    ...
    memset(buffer_respuesta, 0, MAX_HTTP_OUTPUT_BUFFER);
...
static void https_telegram_sendMessage(char mensaje[]) {
    ...
    ESP_LOGW(TAG_HTTP, "Enviare POST");
