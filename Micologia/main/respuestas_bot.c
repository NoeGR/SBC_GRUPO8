...
// Función para procesar los comandos del bot y generar respuestas
int respuestas_bot(char pregunta[], char *mensaje, float datos[]) {
    ...
    if (sscanf(pregunta, "/config_riego %f", &nuevo_umbral) == 1) {
        ...
        datos[4] = nuevo_umbral;  // Actualizar el umbral en el sistema
    ...
    // Comando no reconocido
        ...
        opcion = 0;  // Sin acción asociada
    }

    // Copiar el mensaje generado al buffer de salida
    strncpy(mensaje, aux, 200);
    return opcion;
}

// Función para enviar mensajes personalizados relacionados con el riego
